  ...
  function updateFilePaths(filePath) {
    ...
    content = content.replace(dataLayersRegex, (match, jsonStr) => {
      try {
        ...
        const encodedLayers = JSON.stringify(updatedLayers).replace(
          /"/g,
          "&quot;"
        );
        ...
        console.error(
          `Failed to parse data-layers JSON in file ${filePath}:`,
          error
        );
    ...
    content = content.replace(dataBlockRegex, (match, jsonStr) => {
      try {
        ...
        const encodedBlock = JSON.stringify(updatedBlock).replace(
          /"/g,
          "&quot;"
        );
        ...
        console.error(
          `Failed to parse data-block JSON in file ${filePath}:`,
          error
        );
        return match; // Return original if parsing fails
      }
